...
void Uoptionwidget::NativeConstruct()
{
	...
	Button_close->OnClicked.AddDynamic(this, &Uoptionwidget::OnClickedclose);

	// 콤보박스에서 선택이 변경될 때의 델리게이트 설정
	ComboBox_Resolution->OnSelectionChanged.AddDynamic(this, &Uoptionwidget::OnResolutionChanged);
     ...
     if (ComboBox_ScreenMode)
    {
        ...
        // 콤보박스 값이 변경될 때 호출될 함수를 바인딩
         ComboBox_ScreenMode->OnSelectionChanged.AddDynamic(this, &Uoptionwidget::OnScreenModeChanged);
...
void Uoptionwidget::PopulateResolutionComboBox()
{
    // 예시 해상도 옵션을 추가
...
void Uoptionwidget::ApplyResolution(FVector2D Resolution)
{
...
void Uoptionwidget::ApplyScreenMode(const FString& SelectedMode)
{
    UGameUserSettings* UserSettings = GEngine->GetGameUserSettings();
    if (UserSettings)
    {
...
void Uoptionwidget::OnClickedclose()
{
	SetVisibility(ESlateVisibility::Hidden);
}
void Uoptionwidget::OnScreenModeChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
